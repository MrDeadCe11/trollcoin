...
contract TrollcoinTest is Test {
    ...
    //     vm.startPrank(DEPLOYER);
    //     assertTrue(trollcoin.transfer(from, 100));
    //     vm.stopPrank();
    //     // trollcoin.mint(from, 1e18);

    //     vm.prank(from);
    //     trollcoin.approve(address(this), 100);

    //     assertTrue(trollcoin.transferFrom(from, address(0xBEEF), 100));
    //     assertEq(trollcoin.totalSupply(), 1e18);

    //     assertEq(trollcoin.allowance(from, address(this)), 0);

    //     assertEq(trollcoin.balanceOf(from), 0);
    //     assertEq(trollcoin.balanceOf(address(0xBEEF)), 10000100);
    // }
